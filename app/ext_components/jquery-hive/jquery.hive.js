...
(function(jQuery) {
  ...
  jQuery.Hive = (function() {
    ...
    function _addMessageListener(worker, callback) {
      worker.addEventListener("message", function(event) {
        var fn = callback, response = event.data, _msg;
    ...
    return {
      /**
       * jQuery.Hive.options -> Setup properties for jQuery.Hive.create(
       * [options] )
       */
      options : {
        /**
         * jQuery.Hive.options.count -> Set property from jQuery.Hive.create( {
         * count: [int] } ), number of threads to create
         */
        count : 1,
        /**
         * jQuery.Hive.options.worker -> Set string property from
         * jQuery.Hive.create( { worker: [file-name] } ), name of worker file
         */
        worker : "",
        /**
         * jQuery.Hive.options.receive -> Set callback from jQuery.Hive.create( {
         * receive: [callback] } ), callback to execute when worker receives
         * message (can be global or worker specific)
         */
        receive : jQuery.noop,
        /**
         * jQuery.Hive.options.created -> Set callback from jQuery.Hive.create( {
         * created: [callback] } ), callback to execute when workers have been
         * created
         */
        created : jQuery.noop,
        /**
         * jQuery.Hive.options.special -> NOT IMPLEMENTED/INCOMPLETE - Set
         * callback as a second argument to $().send()
         */
        special : "",
        /**
         * jQuery.Hive.options.error() -> NOT IMPLEMENTED/INCOMPLETE - Error
         * handler
         */
      ...
      /**
       * jQuery.Hive.create( options ) -> Array, create workers, returns array
       * of all workers
       */
      create : function(options) {
        ...
        var i = 0, _options = jQuery.extend({}, jQuery.Hive.options, options);
        _options.count = options.count ? options.count : 1;
        // If threads exist, new threads to cache
        if (_threads.length > 0) {
          // force thread count starting position to avoid overwriting existing
          // threads
          i = _lastThreadInt + 1;
      ...
      /**
       * jQuery.Hive.get( id ).send( message, callback ) -> Send [message] to
       * worker thread, set optional receive callback --> SIMPLER ALTERNATIVE:
       * $.Hive.get(id).send( [message], function() {} ) --> Allows for passing
       * a jQuery.Hive.get(id) object to $() ie. $( $.Hive.get(id) ).send(
       * [message] )
       */
      ...
      /**
       * jQuery.Hive.get( [id] ) -> Return all or specified worker by [id], [id]
       * argument is optional --> $.Hive.get() returns all worker objects in the
       * $.Hive --> $.Hive.get(1) returns the worker object whose ID is 1
       */
      get : function(id) {
        if (id !== undefined) {
  ...
  /**
   * jQuery.( $.Hive.get( id ) ).send( message ) -> Send [message] to worker
   * thread --> SIMPLER ALTERNATIVE: $.Hive.get(id).send( [message] ) --> Allows
   * for passing a $.Hive.get(id) object to $() ie. $( $.Hive.get(id) ).send(
   * [message] )
   */
