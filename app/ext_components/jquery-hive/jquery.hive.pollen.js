 ...
 * http://github.com/rwldrn http://pollenjs.com MIT License version: 0.1.91 (see
 * source for other credits)
 */
(function(WorkerGlobalScope) {
  ...
  // Aliases
  each = Array.prototype.forEach, indexOf = Array.prototype.indexOf, push = Array.prototype.push, slice = Array.prototype.slice, hasOwn = Object.prototype.hasOwnProperty, toStr = Object.prototype.toString, trim = String.prototype.trim;
  Pollen.prototype = {
    identity : 0,
    identify : {
      identify : function() {
    ...
    date : {
       ...
       */
      now : function(uts) {
        return uts ? Math.round(new Date().getTime() / 1000) : +new Date();
      }
    },
    /**
     * Function
    ...
    func : {
      /**
       * $.noop() -> Empty function, Lifted from jQuery 1.4+, returns empty
       * function(replaced $.emptyFn())
       */
      noop : function() {
      },
      /**
       * $.bind( function, context, args ) -> Function with explicit context,
       * context sets 'this'
       */
    ...
    evaluate : {
      ...
      isObj : function(arg) {
        ...
        if (arg.constructor && !hasOwn.call(arg, "constructor")
    ...
    array : {
      /**
       * $.each( object, function ) -> Enumerate [object] (object,array), apply
       * [function]
       */
      ...
      /**
       * $.isAtIndex( array, index, needle ) -> Boolean, Checks if [needle] is
       * located at [index] in [array]
       */
      ...
      inArray : function(a, b) {
        var key, needle = Array.isArray(a) ? b : a, haystack = Array.isArray(a) ? a : b;
        return haystack.indexOf(needle) > -1;
      },
      /**
       * $.clone( arg ) -> Array || Object, clone of [array] or Object, deep
       * clone of [object]
       */
      clone : function(arg) {
        if (Pollen.evaluate.isArr(arg)) {
      ...
      /**
       * $.unique( arg ) -> Array || Object, removes duplicate values from
       * [arg](array||object), returns uniques
       */
      unique : function(arg) {
        var i = 0, ret = [], cache = [];
        for ( var val in arg) {
      ...
      merge : function(arr) {
        ...
        for (; i < arguments.length; i++) {
          merged = Pollen.evaluate.isArr(arguments[i]) ? merged.concat(arguments[i]) : Pollen.object.extend(merged,
              arguments[i]);
        }
        return Pollen.evaluate.isArr(merged) ? this.unique(merged) : merged;
      },
      /**
       * $.combine( keys, values ) -> Object, An object, where [keys](array) =
       * [values](array)
       */
      ...
      /**
       * $.filter( arg , function, iteration ) -> (Array||Object), apply
       * [function](true||false) to each item in [arg], return (array||object)
       * of true
       */
      ...
      /**
       * $.map( array, function ) -> Array, apply [function] to each item in
       * [array], return array of new values
       */
      map : function(arr, fn) {
      ...
      /**
       * $.grep( array, expression, function ) -> Object, values that match
      ...
      compact : function(arg) {
        var _filter = [ undefined, null, false ], ret = Pollen.evaluate.isArr(arg) ? [] : {}, _compact = function(value) {
    ...
    object : {
      ...
      /**
       * $.extend( object, _object ) -> Object, copy properties fron [_object]
       * to [object]
       */
      extend : function(obj, src) {
        ...
        for ( var prop in src) {
          if (hasOwn.call(src, prop)) {
            var srcProp = src[prop], objProp = obj[prop];
            if (!Pollen.evaluate.isNull(srcProp)) {
              obj[prop] = (objProp && typeof srcProp == "object" && typeof objProp == "object") ? Pollen.array.merge(
                  objProp, srcProp) : srcProp;
            }
          }
        }
        return obj;
    ...
    ajax : {
       ...
       * request.url -> url to open --> request.data -> params -->
      ...
      _ajax : function(request) {
        var options = Pollen.object.extend(this._options, request), json = options.dataType == "json" ? true : false, // change
        // to
        // accept
        // data-types
        // not specifically json.
        _type = options.type.toLowerCase(), _xhr = options.xhr(), ajaxSuccess = options.success;
    ...
    data : {
      /**
       * $.param( arg ) -> String, Derived and Adapted from, similar in behavior
       * to jQuery.param()
       */
      param : function(arg) {
      ...
      /**
       * basic storage, needs A LOT of work basically right now this is a place
      ...
      /**
       * $.storage( key, val ) -> ?, persistant across worker messages (by
       * worker) data storage - setter $.storage( key ) -> Val, persistant
       * across worker messages (by worker) data storage - getters
       */
      ...
      /*
       * http://goessner.net/articles/JsonPath/ C. Jason E. Smith, Apache
       * License, Version 2.0 (the "License"); OVERVIEW: JSONPath api
       * (http://goessner.net/articles/JsonPath/) - Similar to XQuery.
       * JSONQuery.js By Jon Crosby
       */
      /**
       * $.query( selector, object ) -> *, Query json or object with JSONPath
       * selector --> http://goessner.net/articles/JsonPath/
       */
      query : (function() {
        // Special case slice
        ...
        function expand(obj, name) {
          ...
          // name can be a property to search for, undefined for full recursive,
          // or an
          // array for picking by index
          var ret = [];
          function walk(obj) {
            if (name) {
            ...
            for ( var i in obj) {
              var val = obj[i];
              if (!name) {
                // if we don't have a name we are just getting all the
          ...
          if (Pollen.evaluate.isArr(name)) {
            ...
            if (name.length == 1) {
              ...
              // about commas
              // in the brackets like [@.func(4,2)]. Fixing the parser would
              // require recursive
        ...
        return function(query, obj) {
          ...
          function pcall(name) {
            // creates a function call and puts the current expression in a
            // parameter for a
            // call
            prefix = name + "(" + prefix;
          }
          function makeRegex(t, a, b, c, d) {
            ...
            return strs[d].match(/[\*\?]/) || c == "~" ? "/^"
                + strs[d].substring(1, strs[d].length - 1).replace(/\\([btnfr\\"'])|([^\w\*\?])/g, "\\$1$2").replace(
                    /([\*\?])/g, ".$1") + (c == "~" ? "$/i" : "$/") + ".test(" + a + ")" : t;
          }
          query = query.replace(/"(\\.|[^"\\])*"|'(\\.|[^'\\])*'|[\[\]]/g, function(t) {
            depth += t == "[" ? 1 : t == "]" ? -1 : 0; // keep track of bracket
            // depth
            return (t == "]" && depth > 0) ? "`]" : // we mark all the inner
            // brackets as
            // skippable
            (t.charAt(0) == '"' || t.charAt(0) == "'") ? "`" + (strs.push(t) - 1) : // and
            // replace
            // all
            // the
            // strings
          ...
          .replace(/([^<>=]=)([^=])/g, "$1=$2").replace(/@|(\.\s*)?[a-zA-Z\$_]+(\s*:)?/g, function(t) {
            return t.charAt(0) == "." ? t : // leave .prop alone
            t == "@" ? "$obj" : // the reference to the current object
            (t.match(/:|^(\$|Math|true|false|null)$/) ? "" : "$obj.") + t; // plain
            // names
            // should be
            // properties of
            // root...
            ...
            // initializer
              ...
              function(t, a, b) {
                ...
                if (oper) {
                  ...
                  pcall(oper[1].match(/\=/) ? "Pollen.array.map" : oper[1].match(/\^/) ? "distinctFilter"
                      : "Pollen.array.filter");
                  return prefix + ",function($obj){return " + oper[2] + "})";
                }
                oper = t.match(/^\[\s*([\/\\].*)\]/); // [/sortexpr,\sortexpr]
                if (oper) {
                  // make a copy of the array and then sort it using the sorting
                  // expression
                  return ".concat().sort(function(a,b){"
                      + oper[1].replace(/\s*,?\s*([\/\\])\s*([^,\\\/]+)/g, function(t, a, b) {
                        return "var av= " + b.replace(/\$obj/, "a") + ",bv= " + b.replace(/\$obj/, "b") + // FIXME:
                        // Should
                        // check
                        // to
                        // make
                ...
                oper = t.match(/^\[(-?[0-9]*):(-?[0-9]*):?(-?[0-9]*)\]/); // slice
                // [0:3]
                if (oper) {
                  pcall("slice");
          ...
          .replace(/`([0-9]+)\s*(==|~)\s*(\$obj(\s*\.\s*[\w_$]+)*)/g, function(t, a, b, c, d) { // and
            // do
            // it
            // for
            // reverse
          ...
          // create a function within this scope (so it can use expand and
          // slice)
    ...
    worker : {
      /**
       * TODO - NEW FEATURE - Add Support for workers to spawn new workers. -
       * Add Support for EventSource
       */
      /**
       * $.send( message ) -> Wraps and normalizes postMessage() across
       * supporting clients, sends [message](object||string||array) to client
       * --> if [message] is an object, and message.SEND_TO & message.SEND_FROM
       * is defined, the message will be sent directly to the worker defined by
       * SEND_TO
       */
      ...
      /**
       * $.receive( function ) -> Worker, execute [function] when worker
       * receives a message from the client --> first argument is message object
       * --> 'this' is WorkerGlobalScope --> can be shortened to
       * $(function(message) { } );
       */
      receive : function(fn) {
    ...
    json : {
      /**
       * $.encode( arg ) -> JSON Object, turns [arg] into JSON (Convenience
       * shorthand)
       */
      encode : JSON.parse,
      /**
  ...
  ("identify date string evaluate reflection array func object data json worker").split(" ").forEach(function(obj) {
